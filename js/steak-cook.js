...
function prepSlides(m){
  ...
  s.push({image:A.stkMise,kicker:"MISE EN PLACE",title:"Everything on the bench",body:`Tonight's protocol: ${m.cut.label}, ${m.th.cm} cm, ${m.gr.label.toLowerCase()}, taken to ${m.dn.label.toLowerCase()} (${m.dn.band}) by ${m.me.label.toLowerCase()}. You need: ${m.me.gear.toLowerCase()}. A probe thermometer is not optional — colour lies, the probe doesn't.`});
  return s;
}
function cookSlides(m){
  ...
  if(m.me.id==="pan"){
    ...
    s.push({image:A.stkReverseRack,kicker:"THE SLOW PHASE",title:"Low oven, wire rack",body:`Oven at 100–120°C, steak on a wire rack over a tray, probe in the core. You're waiting for ${m.pull}°C — roughly ${Math.round(m.th.cm*11)} minutes for ${m.th.cm} cm, but the probe is the only clock that matters.`});
