...
import { courses, storySlides } from "./data.js";
...
function img(src, alt="", cls=""){
  return `<img class="${cls}" src="${src}" alt="${alt}" loading="lazy" decoding="async" onerror="this.replaceWith(document.querySelector('#imageFallback').content.cloneNode(true))">`;
}
function route(name){
...
function home(){
 return `<div class="home-layout">
  <section class="hero card">
    ${img(assets.heroEditorial,"Premium cheeseburger")}
    <div class="hero-copy">
...
function openStory(id){
 const slides=storySlides[id]; if(!slides)return;
 let index=0;
 const story=document.createElement("section"); story.className="story";
 const draw=()=>{
   const s=slides[index];
   story.innerHTML=`<div class="story-progress">${slides.map((_,i)=>`<i class="${i<=index?"active":""}"></i>`).join("")}</div>
   <button class="story-close" aria-label="Close">×</button>
   <div class="story-slide">${img(s.image,s.title)}
    <div class="story-content"><p class="eyebrow" style="color:#fff">${s.kicker}</p><h2>${s.title}</h2><p>${s.body}</p></div>
    <div class="story-tap"><button aria-label="Previous"></button><button aria-label="Next"></button></div>
   </div>`;
   story.querySelector(".story-close").onclick=()=>story.remove();
   const [prev,next]=story.querySelectorAll(".story-tap button");
   prev.onclick=()=>{index=Math.max(0,index-1);draw()};
   next.onclick=()=>{if(index===slides.length-1){if(!state.completed.includes(id))state.completed.push(id);save();story.remove();render()}else{index++;draw()}};
 };
 document.body.appendChild(story); draw();
}
